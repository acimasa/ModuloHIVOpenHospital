...
public class IoOperations {
	...
	public Disease getDiseaseByCode(int code) throws OHException {
		...
		try{
			...
			while (resultSet.next()) {
				...
				disease.setIpdInclude(resultSet.getInt("DIS_IPD_INCLUDE")==1);
			}
	...
	/**
	 * Retrieves all stored {@link Disease}s with flag odp equals <code>true</code>.
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Disease> getDiseaseOpd() throws OHException {
		return getDiseases(null,true,false);
	}

	/**
	 * Retrieves all stored {@link Disease}s.
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Disease> getAllDiseases() throws OHException {
		return getDiseases(null,false,false);
	}

	/**
	 * Retrieves all stored {@link Disease}s with flag odp equals <code>true</code> and the specified disease type code.
	 * @param disTypeCode the disease type code.
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Disease> getDiseaseOpd(String disTypeCode) throws OHException {
		return getDiseases(disTypeCode,true,false);
	}

	/**
	 * Retrieves all stored {@link Disease}s with flag ipd equals <code>true</code>.
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Disease> getDiseaseIpd() throws OHException {
		return getDiseases(null,false,true);
	}

	/**
	 * Retrieves all stored {@link Disease}s with flag idp equals <code>true</code> and the specified disease type code.
	 * @param disTypeCode the disease type code.
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Disease> getDiseaseIpd(String disTypeCode) throws OHException {
		return getDiseases(disTypeCode,false,true);
	}

	/**
	 * Retrieves all stored {@link Disease}s with the specified disease type code.
	 * @param disTypeCode the disease type code.
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Disease> getDisease(String disTypeCode) throws OHException {
		return getDiseases(disTypeCode,false,false);
	}

	/**
	 * Retrieves stored disease with the specified search parameters.
	 * @param disTypeCode not null apply to disease type
	 * @param opdOnly select only diseases related to out patient
	 * @param ipdOnly select only diseases related to in patient
	 * @return the retrieved diseases.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	private ArrayList<Disease> getDiseases(String disTypeCode, boolean opdOnly, boolean ipdOnly) throws OHException {
		ArrayList<Disease> diseases = null;
		String selectClause = "select * from DISEASE join DISEASETYPE on DIS_DCL_ID_A = DCL_ID_A";
		String whereClause = "";
		//System.out.println(MessageBundle.getMessage("angal.disease.here") + "="+disTypeDescription);
		...
		if (ipdOnly) {
			if (whereClause.equals("")) whereClause=" where "; else whereClause+=" and  "; 
			whereClause+=" (DIS_IPD_INCLUDE=1 or DIS_IPD_INCLUDE is null) ";
		}
	...
	public boolean newDisease(Disease disease) throws OHException{
		...
		try{
			...
			parameters.add(disease.getIpdInclude());

			String query = "insert into DISEASE (DIS_ID_A, DIS_DCL_ID_A, DIS_DESC, DIS_OPD_INCLUDE, DIS_IPD_INCLUDE) values (?, ?, ?, ?, ?)";
	...
	public boolean updateDisease(Disease disease) throws OHException {
		...
		try{
			...
			String query = "UPDATE DISEASE SET DIS_DCL_ID_A = ?, DIS_DESC = ?, DIS_OPD_INCLUDE = ?, DIS_IPD_INCLUDE = ?," +
	...
	public boolean deleteDisease(Disease disease) throws OHException{
		...
		try{
			List<Object> parameters = Collections.<Object>singletonList(disease.getCode());
			String query = "UPDATE DISEASE SET DIS_OPD_INCLUDE = 0, DIS_IPD_INCLUDE = 0 WHERE DIS_ID_A = ?";
			result = dbQuery.setDataWithParams(query, parameters, true);
		} finally{
			dbQuery.releaseConnection();
		}
