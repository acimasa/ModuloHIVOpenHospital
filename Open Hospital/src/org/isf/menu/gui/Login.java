...
public class Login extends JDialog implements ActionListener, KeyListener {
	...
	private void acceptPwd(){
		...
		boolean found = false;
		for (User u :  users){
			if (u.getUserName().equals(userName)&&u.getPasswd().equalsIgnoreCase(passwd)){
				returnUser = u;
				found = true;
			}
		}
	...
	private class LoginPanel extends JPanel {
		...
		public LoginPanel(Login myFrame) {
			...
			*/
			...
			usersList.setPreferredSize(new Dimension(280,d.height));
			
			JPanel userPanel = new JPanel(new FlowLayout(FlowLayout.CENTER,5,10));
			userPanel.add(usersList);
			
			pwd = new JPasswordField(19);
			pwd.setName("pwd");
			pwd.addKeyListener(myFrame);
			
			JPanel pwdPanel = new JPanel(new FlowLayout(FlowLayout.CENTER,5,5));
			pwdPanel.add(new JLabel(MessageBundle.getMessage("angal.menu.password")));
			pwdPanel.add(pwd);
