...
public class IoOperations {
	...
	public boolean updateUser(User user) throws OHException {
		...
		String sqlstring = "UPDATE USER SET US_PASSWD = ?, US_DESC = ? WHERE US_ID_A = ?";
		parameters.add(user.getPasswd());
		parameters.add(user.getDesc());
		parameters.add(user.getUserName());
	 ...
	 * @throws OHException 
	 */
	public boolean setGroupMenu(UserGroup aGroup, ArrayList<UserMenuItem> menu) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = new ArrayList<Object>();
	...
	public boolean newUserGroup(UserGroup aGroup) throws OHException {
		...
		result = result && setGroupMenu(aGroup, menu);
		return result;
	}
