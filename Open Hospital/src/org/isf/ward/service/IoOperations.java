...
public class IoOperations {
	...
	/**
	 * Retrieves all stored {@link Ward}s with the specified disease type code.
	 * 
	 * @return the retrieved wards.
	 * @throws OHException if an error occurs retrieving the diseases.
	 */
	public ArrayList<Ward> getWard() throws OHException {
		return getWard(null);
	}
		...
		try {
			...
			while (resultSet.next()) {
				pWard.add(new Ward(resultSet.getString("WRD_ID_A"), resultSet.getString("WRD_NAME"), 
						...
						resultSet.getBoolean("WRD_IS_PHARMACY"), resultSet.getInt("WRD_LOCK")));
			}
	 ...
	 */
	public ArrayList<Ward> getWard(String wardID ) throws OHException {
		ArrayList<Ward> pWard = null;
	...
	public boolean newWard(Ward ward) throws OHException {
		...
		try {
			String sqlstring = "INSERT INTO WARD (WRD_ID_A, WRD_NAME, WRD_TELE, WRD_FAX, WRD_EMAIL," +
					" WRD_NBEDS, WRD_NQUA_NURS, WRD_NDOC, WRD_IS_PHARMACY) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
			Object[] objects = {ward.getCode(),ward.getDescription(),ward.getTelephone(),ward.getFax(),ward.getEmail(),ward.getBeds(),ward.getNurs(),ward.getDocs(),ward.isPharmacy()? 1:0};
