...
public class AdmissionBrowser extends JDialog {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (wardBox.getSelectedIndex() == 0) {
						...
						if (wardId.equalsIgnoreCase(admission.getWardId())) {
							...
							if (freeBeds <= 0)
								JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.wardwithnobedsavailable"));
						}
					}
	...
	private void updateBedDays() {
		try {
			...
			int bedDays = (int) ((discharge.getTime() - admission.getTime()) / 86400000);
		...
		if (saveButton == null) {
			...
			saveButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (wardBox.getSelectedIndex() == 0) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseselectavalidward"));
						return;
					} else {
					...
					if (diseaseInBox.getSelectedIndex() == 0) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseselectavaliddiseasein"));
						return;
					} else {
						try {
							admission.setDiseaseInId(diseaseList.get(diseaseInBox.getSelectedIndex() - 1).getCode());
					...
					if (disease1index == 0) {
						...
						admission.setDiseaseOutId1(diseaseList.get(disease1index - 1).getCode());
					}
					...
					if (disease2index == 0) {
						...
						admission.setDiseaseOutId2(diseaseList.get(disease2index - 1).getCode());
					}
					...
					if (disease3index == 0) {
						...
						admission.setDiseaseOutId3(diseaseList.get(disease3index - 1).getCode());
					}
					...
					try {
						...
						if (x < 0) {
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertacorrectprogressiveid"));
							return;
						} else {
					...
					try {
						...
						if (dateIn.after(today)) {
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.futuredatenotallowed"));
						...
						if (dateIn.before(today)) {
							...
							for (Admission ad : admList) {
								...
								if ((ad.getAdmDate().before(dateIn) || ad.getAdmDate().compareTo(dateIn) == 0) 
									...
									JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.ininserteddatepatientwasalreadyadmitted"));
									d = currentDateFormat.format(today);
									dateInFieldCal.setDate(currentDateFormat.parse(d));
									return;
								}
						...
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavalidadmissiondate"));
						//dateInField.setText(currentDateFormat.format(dateIn.getTime()));// CONTROLLARE
						return;
					...
					if (admTypeBox.getSelectedIndex() == 0) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseselectavalidadmissiondate"));
						return;
					} else {
					...
					if (d.equals("")) {
						...
						try {
							...
							if (dateOut.before(dateIn)) {
								JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.dischargedatemustbeafteradmissiondate"));
								return;
							}
								...
								for (Admission ad : admList) {
									...
									if (ad.getDisDate() == null) {
										if (!dateOut.after(ad.getAdmDate()))
											...
											JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.intheselecteddatepatientwasadmittedagain"));
											return;
										}
									}
									// general case
							...
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavaliddischargedate"));
							return;
						} catch (IllegalArgumentException iae) {
					...
					if (d.equals("")) {
						...
						try {
							...
							if (operationDate.before(dateIn) || operationDate.after(limit)) {
								JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavalidvisitdate"));
								return;
							}
					...
					if (disTypeBox.getSelectedIndex() == 0) {
						if (isDischarge) {
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseselectavaliddischargetype"));
							return;
						} else {
						...
						if (dateOut == null) {
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertadischargedate"));
							return;
						}
						if (isDischarge) {
					...
					if (admission.getDiseaseOutId1() == null) {
						if (isDischarge) {
							...
							if (yes == JOptionPane.YES_OPTION) {
								...
								JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseselectatleastfirstdiagnosisout"));
								return;
							}
						}
					} else {
					...
					try {
						...
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavalidunitvalue"));
						return;
					}
					...
					if (isPregnancy) {
						...
						if (d.equals("")) {
							...
							try {
								...
								if (admission.getDeliveryDate() == null) {
									JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.controln1datenodeliverydatefound"));
									return;
								}
								GregorianCalendar limit;
								if (admission.getDisDate() == null) {
								...
								if (ctrl1Date.before(deliveryDate) || ctrl1Date.after(limit)) {
									JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavalidcontroln1date"));
									return;
								}
						...
						if (d.equals("")) {
							...
							if (admission.getCtrlDate1() == null) {
								JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.controldaten2controldaten1notfound"));
							...
							try {
								...
								if (ctrl2Date.before(ctrl1Date) || ctrl2Date.after(limit)) {
									JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavalidcontroln2date"));
									return;
								}
						...
						if (d.equals("")) {
							...
							try {
								...
								if (ctrl2Date != null && abortDate.before(ctrl2Date) || ctrl1Date != null && abortDate.before(ctrl1Date) || abortDate.before(visitDate) || abortDate.after(limit)) {
									JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.pleaseinsertavalidabortdate"));
									return;
								}
					...
					if (!editing && !isDischarge) {
						int newKey = abm.newAdmissionReturnKey(admission);
						if (newKey > 0) {
							result = true;
					...
					if (!result) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.admission.thedatacouldnotbesaved"));
					} else {
						dispose();
					}
				}
