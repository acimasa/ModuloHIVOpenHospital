...
public class WardPharmacy extends ModalJFrame implements WardPharmacyEdit.MovementWardListeners, WardPharmacyNew.MovementWardListeners {
	...
	private String[] columsIncomes = { MessageBundle.getMessage("angal.medicalstockward.date"), MessageBundle.getMessage("angal.medicalstockward.medical"),
	...
	private String[] columsOutcomes = { MessageBundle.getMessage("angal.medicalstockward.date"), MessageBundle.getMessage("angal.medicalstockward.patient"),
		...
		if (jButtonNew == null) {
			...
			jButtonNew.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					// CREATE MOVEMENT BY DRUG
					/*
					 * if (jTableDrugs.getSelectedRow() >= 0) { drugSelected =
					 * (Medical) ((jTableDrugs.getModel())
					 * .getValueAt(jTableDrugs.getSelectedRow(), -1));
					 * WardPharmacyEdit editor = new
					 * WardPharmacyEdit(wardSelected, drugSelected);
					 * editor.addMovementWardListener(WardPharmacy.this);
					 * editor.setVisible(true); } else {
					 */
					WardPharmacyNew editor = new WardPharmacyNew(wardSelected, wardDrugs);
					editor.addMovementWardListener(WardPharmacy.this);
			...
			jButtonEdit.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (jTableOutcomes.getSelectedRow() < 0 || !jScrollPaneOutcomes.isShowing()) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicalstockward.pleaseselectanoutcomesmovementfirst"), MessageBundle.getMessage("angal.hospital"),
	...
	private JButton getFilterButton() {
		if (filterButton == null) {
			...
			filterButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (ageFrom > ageTo) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicalstockward.agefrommustbelowerthanageto"));
						jAgeFromTextField.setText(String.valueOf(ageTo));
						ageFrom = ageTo;
						return;
					}
					if (weightFrom > weightTo) {
						JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicalstockward.weightfrommustbelowerthanweightto"));
				...
				public void focusLost(FocusEvent e) {
					try {
						...
						if ((weightTo < 0) || (weightTo > 200)) {
							...
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicalstockward.insertavalidweight"));
						}
				...
				public void focusLost(FocusEvent e) {
					try {
						...
						if ((ageTo < 0) || (ageTo > 200)) {
							...
							JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.medicalstockward.insertvalidage"));
						}
					} catch (NumberFormatException ex) {
						jAgeToTextField.setText("0");
					}
	...
	private JScrollPane getJScrollPaneOutcomes() {
		if (jScrollPaneOutcomes == null) {
			...
			// jScrollPaneOutcomes.setBounds(8, 100, 652, 150);
	...
	class IncomesModel extends DefaultTableModel {
		 ...
		 */
		private static final long serialVersionUID = 1L;
		ArrayList<Movement> wardIncomes;

		public IncomesModel() {
			wardIncomes = new ArrayList<Movement>();
			listMovementCentral = movManager.getMovements(wardSelected.getCode());
		...
		public Object getValueAt(int r, int c) {
			...
			if (c == 2) {
				...
				if (pcsPerPck != 0) {
					...
					//return packets + " " + MessageBundle.getMessage("angal.medicalstockward.packets") + " (" + pieces + " " + MessageBundle.getMessage("angal.medicalstockward.pieces") + ")"; //$NON-NLS-1$
					return pieces;
				}
			}
			return null;
	...
	class OutcomesModel extends DefaultTableModel {
		 ...
		 */
		private static final long serialVersionUID = 1L;
		ArrayList<MovementWard> wardOutcomes;
		...
		public Object getValueAt(int r, int c) {
			if (c == -1) {
				return wardOutcomes.get(r);
			}
			if (c == 0) {
				return formatDateTime(wardOutcomes.get(r).getDate());
			}
			if (c == 1) {
				return wardOutcomes.get(r).getDescription();
			}
			if (c == 2) {
			...
			if (c == 4) {
				MovementWard mov = wardOutcomes.get(r);
				if (mov.isPatient()) {
					float weight = wardOutcomes.get(r).getWeight();
					return weight == 0 ? MessageBundle.getMessage("angal.medicalstockward.notdefined.abb") : weight;
				} else
					return MessageBundle.getMessage("angal.medicalstockward.notapplicable.abb");
			}
			if (c == 5) {
				return wardOutcomes.get(r).getMedical().getDescription();
			}
			if (c == 6) {
				return "" + wardOutcomes.get(r).getQuantity() + " " + //$NON-NLS-1$ 
						wardOutcomes.get(r).getUnits();
			}
			return null;
		}
	...
	class DrugsModel extends DefaultTableModel {
		...
		public Object getValueAt(int r, int c) {
			if (c == -1) {
				return wardDrugs.get(r);
			}
			if (c == 0) {
				return wardDrugs.get(r).getMedical().getDescription();
			}
			if (c == 1) {
				return wardDrugs.get(r).getQty() + " " + MessageBundle.getMessage("angal.medicalstockward.pieces");
			}
			return null;
		}
