...
public class IoOperations {
	...
	public ArrayList<Opd> getOpdList(String diseaseTypeCode,String diseaseCode, GregorianCalendar dateFrom,GregorianCalendar dateTo,int ageFrom, int ageTo,char sex,String newPatient) throws OHException{
		...
		sqlString.append("SELECT * FROM OPD LEFT JOIN DISEASE ON OPD_DIS_ID_A = DIS_ID_A LEFT JOIN DISEASETYPE ON DIS_DCL_ID_A = DCL_ID_A WHERE 1");
	...
	public ArrayList<Opd> getOpdList(int patID) throws OHException {
		...
		if (patID == 0) {
			String sqlString = "SELECT * FROM OPD ORDER BY OPD_PROG_YEAR DESC";
			resultSet = dbQuery.getData(sqlString, true);
		} else {
			String sqlString = "SELECT * FROM OPD WHERE OPD_PAT_ID = ? ORDER BY OPD_PROG_YEAR DESC";
	...
	public boolean newOpd(Opd opd) throws OHException{
		...
		sqlString.append(" OPD_PAT_FULLNAME,");
		sqlString.append(" OPD_PAT_FNAME,");
		sqlString.append(" OPD_PAT_SNAME,");
		sqlString.append(" OPD_PAT_NEXT_KIN,");
		sqlString.append(" OPD_PAT_ADDR,");
		sqlString.append(" OPD_PAT_CITY");
		sqlString.append(") ");
