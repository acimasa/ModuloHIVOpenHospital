...
public class TherapyEntryForm extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public TherapyEntryForm(JDialog owner, Therapy th) {
		super(owner, true);
		this.therapy = th;
	...
	private void fillCalendarsFromTherapy(Therapy th) {
		...
		int differenzaInGiorni = TimeTools.getDifferenzaInGiorni(firstDay, secondDay);

		jSpinnerFreqInPeriod.setValue(differenzaInGiorni > 0 ? differenzaInGiorni : 1);
	...
	private void fillDaysWeeksMonthsFromDates(GregorianCalendar firstDay, GregorianCalendar lastDay) {
		...
		int mesi = settimane / 4;
		settimane = settimane - mesi * 4;
		
		jSpinnerMonths.setValue(mesi);
		jSpinnerDays.setValue(giorni);
		jSpinnerWeeks.setValue(settimane);
	}
		...
		if (buttonOK == null) {
			...
			buttonOK.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					int patID = 0;		//verrà settato dopo
					int numTherapy = 0; //verrà settato dopo
					...
					int unitID = 0;		//verrà settato dopo
					int freqInDay = getFreqInDay();
					int freqInPeriod = Integer.parseInt(jSpinnerFreqInPeriod.getValue().toString());
					String note = noteTextArea.getText();
					boolean notify = false; //verrà settato dopo
					boolean sms = false; //verrà settato dopo
					
					if (therapy != null) {
						patID = therapy.getPatID();
						numTherapy = therapy.getNumTherapy();
						//unitID = therapy.getUnits(); //TODO;
						notify = therapy.isNotify();
						sms = therapy.isSms();
					}
					thRow = new TherapyRow(patID, numTherapy, startDate, endDate, medical, qty, unitID, freqInDay, freqInPeriod, note, notify, sms);
