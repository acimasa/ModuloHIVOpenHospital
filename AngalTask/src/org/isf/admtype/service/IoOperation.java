...
public class IoOperation {
	...
	public static void main(String[] args) throws OHException {
		AdmissionType dis= new AdmissionType(MessageBundle.getMessage("angal.admtype.mx"),MessageBundle.getMessage("angal.admtype.malattiemodificato"));
		IoOperation ciao = new IoOperation();
		ciao.newAdmissionType(dis);
		ArrayList<AdmissionType> vet = ciao.getAdmissionType();
		for (AdmissionType type : vet) {
			System.out.println(type);
		}
		ciao.updateAdmissionType(dis);
		//ciao.newMedicaldsrstockmovType(dis);
		vet = ciao.getAdmissionType();
		for (AdmissionType type : vet) {
			System.out.println(type);
		};
		System.out.println(ciao.isCodePresent("M8"));
