...
public class DiseaseBrowser extends ModalJFrame implements DiseaseEdit.DiseaseListener {
	...
	class DiseaseBrowserModel extends DefaultTableModel {
		...
		public Object getValueAt(int r, int c) {
			if (c == 0) {
				return pDisease.get(r).getCode();
			} else if (c == -1) {
				return pDisease.get(r);
			} else if (c == 1) {
				return pDisease.get(r).getType().getDescription();
			} else if (c == 2) {
				return pDisease.get(r).getDescription();
			}
			return null;
		}
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, 
	   ...
	   {  
		   ...
		   if(!((Disease)table.getValueAt(row,-1)).getIpdInclude() &&
